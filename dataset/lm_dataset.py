...
class PretrainDataset(Dataset):
    ...
    # 拿到jsonl的每一行，要输出inputs_ids, attention_mask, labels
    # 如何做到？
    # 1.tokennizer把文本转化为input_id
    # 2.加上EOS BOS PAD
    # 3.编写labels 防止PAD参与loss运算
    # 4.编写attention_mask，告诉模型哪些位置是有效的，哪些位置是PAD
    def __getitem__(self, index):
        ...
        tokens = self.tokenizer(
            # 假设jsonl中有一个"text"字段，包含了内容
            str(sample["text"]),
            # 不要自动添加特殊token
            add_special_tokens=False,
            # 预留 BOS + EOS 的位置
            max_length=self.max_length - 2,  
            # 如果长度超过self.max_length，自动剪切
            truncation=True,
        ).input_ids # 提取input_ids
